///
///Created by slkk on 2019/9/17/0017 09:10
///
import 'dart:convert';
...
class UpdateVpnPolicyBloc implements BlocBase {
  final BehaviorSubject<CommonResponseModel> _subject =
      BehaviorSubject<CommonResponseModel>();

  Sink<CommonResponseModel> get inUpdateVpnPolicy => _subject.sink;

  Stream<CommonResponseModel> get outUpdateVpnPolicy => _subject.stream;

  setVpnPolicy(String policy) {
    _doSetVpnPolicy(_subject, policy);
  }

  Future<int> _doSetVpnPolicy(
      BehaviorSubject<CommonResponseModel> subject, String policy) async {
    return await Future.value(CapacitiesService.getInstance().sendMessage(
        Constant.mqttCmdTypeUpdateVpnPolicy,
        ...
        jsonEncode(
          requestModel.CommonRequestModel(
            ...
            requestModel.Parameter(
              ...
              data: requestModel.Data(policy: policy),
            ),
          ),
        ),
        0,
