///
///Created by slkk on 2019/9/17/0017 14:48
///
import 'dart:convert';
...
class RebootBloc implements BlocBase {
  final BehaviorSubject<CommonResponseModel> _subject =
      BehaviorSubject<CommonResponseModel>();

  Sink<CommonResponseModel> get inRebootBloc => _subject.sink;

  Stream<CommonResponseModel> get outRebootBloc => _subject.stream;

  reboot() {
    _doReboot(_subject);
  }

  Future<int> _doReboot(BehaviorSubject<CommonResponseModel> subject) async {
    return await Future.value(CapacitiesService.getInstance().sendMessage(
        Constant.mqttCmdTypeReboot,
        ...
        jsonEncode(
          requestModel.CommonRequestModel(
            ...
            requestModel.Parameter(Constant.mqttCmdTypeReboot,
